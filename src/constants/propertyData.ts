 ...
 * Alineado con la app móvil (Ilyrox) para que filtros, cards y detalles
 * usen los mismos tipos/subtipos/campos que los datos reales en Supabase.
 */
...
export const TIPOS_ENERGIA_KVA = [
  "Monofásica: hasta 25 kVA",
  "Trifásica: 25 a 45 kVA",
  "Industrial: 45 a 150 kVA",
  "Alta capacidad: más de 150 kVA",
] as const;
...
export const esTerreno = (subtipo: string | string[]): boolean => {
  ...
  if (Array.isArray(subtipo)) {
    return subtipo.length > 0 && subtipo.every((s) => s.toLowerCase().includes("terreno"));
  }
...
export const esDepartamento = (subtipo: string | string[]): boolean => {
  if (!subtipo) return false;
  if (Array.isArray(subtipo)) {
    return subtipo.length > 0 && subtipo.every((s) => s.toLowerCase() === "departamento");
  }
  return subtipo.toLowerCase() === "departamento";
};

/** Verifica si el subtipo es una "Casa" (cualquier subtipo cuyo nombre empieza con "casa") */
export const esCasa = (subtipo: string | string[]): boolean => {
  if (!subtipo) return false;
  if (Array.isArray(subtipo)) {
    return subtipo.length > 0 && subtipo.every((s) => s.trim().toLowerCase().startsWith("casa"));
  }
  return subtipo.trim().toLowerCase().startsWith("casa");
};

/** Verifica si el subtipo es "Rancho / Finca" (agrícola con casa habitación) */
export const esRanchoFinca = (subtipo: string | string[]): boolean => {
  if (!subtipo) return false;
  if (Array.isArray(subtipo)) {
    return subtipo.length > 0 && subtipo.every((s) => s.toLowerCase().includes("rancho"));
  }
  return subtipo.toLowerCase().includes("rancho");
};

/** Verifica si es propiedad comercial o industrial */
export const esComercialIndustrial = (tipoPrincipal?: TipoPrincipal | string): boolean => {
  return tipoPrincipal === "comercial" || tipoPrincipal === "industrial";
};

/** Obtiene el label para recámaras según tipo de propiedad */
export const getLabelRecamaras = (tipoPrincipal?: TipoPrincipal | string): string => {
  return esComercialIndustrial(tipoPrincipal) ? "Espacios" : "Recámaras";
};

/**
 * Define qué campos mostrar según el subtipo y el tipo principal.
 * Espejo de la app móvil para que cards/detalles/filtros muestren lo mismo.
 */
export const getCamposVisibles = (
  subtipo: string | string[],
  tipoPrincipal?: TipoPrincipal | string,
) => {
  ...
  return {
    ...
    // Mantenimiento mensual: todas excepto agrícola
    mantenimiento: !isAgricola,

    // Características adicionales
    amueblado: !isTerreno && !isIndustrial && !isComercial && !isAgricola,
    petFriendly: !isTerreno && !isIndustrial && !isComercial && !isAgricola,

    // Secciones por tipo
    frenteFondo: isComercial || isTerreno || esCasa(subtipo),
